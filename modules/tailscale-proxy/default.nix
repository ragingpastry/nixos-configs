...
in
{
  ...
  systemd.services.tailscale-proxy = {
    ...
    serviceConfig = {
      ...
      TimeoutStopSec = 70;
      ExecStart = ''
        ${pkgs.docker} run -p ${cfg.listenIP}:${cfg.proxyPort}:1055 --expose ${cfg.proxyPort} \
          -d --name=tailscale-proxy --rm ${cfg.dockerImage} \
          -e TS_AUTHKEY \
          -e TS_OUTBOUND_HTTP_PROXY_LISTEN=:1055 \
          -e TS_SOCKS5_SERVER=:1055 \
          -e TS_EXTRA_ARGS=--exit-node=${cfg.exitNodeIP}
      '';
      ExecStop = "${pkgs.docker} rm -f tailscale-proxy";
    };
  };
}