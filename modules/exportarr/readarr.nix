{ config, pkgs, lib, ... }:
let
  cfg = config.services.exportarr.readarr;
  api_key_fragment =
    if cfg.api_key != ""
...
in
{
  config = lib.mkIf cfg.enable {
    systemd.services.exportarr_readarr = {
      description = "readarr Exportarr";
      wantedBy = [ "multi-user.target" ];
      script = ''
        ${pkgs.exportarr}/bin/exportarr readarr --port ${cfg.listen_port} --url ${cfg.url} ${api_key_fragment} ${interface} ${additionalMetrics}
      '';
    };
  };
}
