{ config, pkgs, lib, ... }:
let
  cfg = config.services.exportarr.prowlarr;
  api_key_fragment =
    if cfg.api_key != ""
...
in
{
  config = lib.mkIf cfg.enable {
    systemd.services.exportarr_prowlarr = {
      description = "prowlarr Exportarr";
      wantedBy = [ "multi-user.target" ];
      script = ''
        ${pkgs.exportarr}/bin/exportarr prowlarr --port ${cfg.listen_port} --url ${cfg.url} ${api_key_fragment} ${interface} ${additionalMetrics}
      '';
    };
  };
}
